...
volatile bool backendReady = false;
...
volatile uint32_t helloEvents = 0;
volatile uint32_t helloFailuresTotal = 0;

// End-to-end application ACK health.
// The backend sends "ACK:<seq>" for every accepted packet.
...
void resetBackendStateLocked(bool connected) {
  wsConnected = connected;
  backendReady = false;
  sessionActive = false;
  helloPending = connected;
  sessionId[0] = '\0';
  sessionGeneration++;
}
...
void webSocketEvent(
  ...
  switch (type) {
    case WStype_CONNECTED: {
      ...
      resetBackendStateLocked(true);
      wsConnectEvents++;

      portEXIT_CRITICAL(&networkMux);

      // A new connection must never inherit frames from an earlier socket
      // or monitoring session. The network task sends HELLO on its next pass.
      discardNetworkQueue();
      break;
    }
    ...
    case WStype_TEXT: {
      ...
      constexpr size_t START_PREFIX_LENGTH = 6;
      if (length > START_PREFIX_LENGTH &&
          memcmp(payload, "START:", START_PREFIX_LENGTH) == 0) {

        size_t requestedLength = length - START_PREFIX_LENGTH;

        if (requestedLength >= sizeof(sessionId)) {
          Serial.println("Backend START rejected: session ID is too long");
          break;
        }

        char requestedSessionId[sizeof(sessionId)];
        memcpy(
            requestedSessionId,
            payload + START_PREFIX_LENGTH,
            requestedLength);
        requestedSessionId[requestedLength] = '\0';

        bool mayStart;

        portENTER_CRITICAL(&networkMux);

        mayStart = wsConnected && backendReady;

        if (mayStart) {
          // Close the old enqueue generation before draining its frames.
          sessionActive = false;
          sessionId[0] = '\0';
          sessionGeneration++;
        }
        ...
        memcpy(sessionId, requestedSessionId, requestedLength + 1);
        sessionActive = true;
      ...
      constexpr size_t STOP_PREFIX_LENGTH = 5;
      if (length > STOP_PREFIX_LENGTH &&
          memcmp(payload, "STOP:", STOP_PREFIX_LENGTH) == 0) {

        const uint8_t *requestedSession = payload + STOP_PREFIX_LENGTH;
        size_t requestedLength = length - STOP_PREFIX_LENGTH;
        bool stopped = false;

        portENTER_CRITICAL(&networkMux);

        if (sessionActive &&
            requestedLength == strlen(sessionId) &&
            memcmp(requestedSession, sessionId, requestedLength) == 0) {
      ...
      // The backend replies with ASCII: ACK:<uint32 sequence>.
      // ACKs only prove liveness while a session is actively streaming.
      if (length > 4 && memcmp(payload, "ACK:", 4) == 0) {
        uint64_t sequence = 0;
        bool validAck = true;
        ...
        for (size_t i = 4; i < length; i++) {
          ...
          sequence = (sequence * 10) + (payload[i] - '0');
        ...
        if (validAck) {
          ...
          if (sessionActive) {
            lastAckSeq = static_cast<uint32_t>(sequence);
            lastAckMillis = millis();
            ackEvents++;
          }
...
void enqueueForNetwork(
    ...
    size_t length,
    uint32_t sequence,
    uint32_t frameSessionGeneration) {
  ...
  shouldEnqueue =
      ...
      backendReady &&
      sessionActive &&
      frameSessionGeneration == sessionGeneration;
  portEXIT_CRITICAL(&networkMux);

  // Idle/offline batches were intentionally drained, not network failures.
  if (!shouldEnqueue) {
    return;
  }
  ...
  frame.sessionGeneration = frameSessionGeneration;
...
bool isSafeHelloValue(const char *value) {
...
bool sendHello() {
  constexpr size_t HELLO_BUFFER_SIZE = 384;
  char hello[HELLO_BUFFER_SIZE];

  if (!isSafeHelloValue(deviceId) ||
      !isSafeHelloValue(bootId) ||
      !isSafeHelloValue(OBJECTIVE_DEVICE_TOKEN)) {

    Serial.println("HELLO configuration is empty or contains unsupported characters");
    return false;
  }
  ...
  if (written < 0 || (size_t)written >= sizeof(hello)) {
    Serial.println("HELLO configuration exceeds the bounded message buffer");
    return false;
  }

  return webSocket.sendTXT(hello, (size_t)written);
}
...
void networkTask(void *parameter) {
  ...
  while (true) {
    ...
    if (!wifiUp) {
      ...
      if (wsConnected || backendReady || sessionActive || helloPending) {
        resetBackendStateLocked(false);
      } else {
        wsConnected = false;
      }
    ...
    if (!wsStartedForCurrentWiFi) {
      ...
      // actually reaching the backend process and replies are returning.
    ...
    if (connected) {
      wsConnected = true;
    } else if (wsConnected || backendReady || sessionActive || helloPending) {
      resetBackendStateLocked(false);
    }

    bool helloNeeded = helloPending;
    bool streaming = sessionActive;
    uint32_t ackMillisSnapshot = lastAckMillis;
    uint32_t sessionGenerationSnapshot = sessionGeneration;
    ...
    if (helloNeeded) {
      bool helloSent = sendHello();

      portENTER_CRITICAL(&networkMux);

      if (helloSent && wsConnected) {
        helloPending = false;
        helloEvents++;
      } else {
        helloFailuresTotal++;
    ...
    // Connected and authenticated can intentionally remain idle forever.
    // Keep servicing WebSocket control traffic without expecting ACKs.
    if (!streaming) {
      discardNetworkQueue();
      vTaskDelay(pdMS_TO_TICKS(1));
      continue;
    ...
    if (xQueueReceive(
      ...
      // A frame serialized during a concurrent START/STOP transition is
      // tagged with its enqueue generation and can never cross sessions.
      if (frame.sessionGeneration != sessionGenerationSnapshot) {
        portENTER_CRITICAL(&networkMux);
        recoveryQueueDropsTotal++;
        portEXIT_CRITICAL(&networkMux);
        continue;
      }
...
void setup() {
  ...
  Serial.println();
  Serial.println("==========================================");
  Serial.println("ESP32 FIVE-SENSOR ACQUISITION V5.3");
  Serial.println("BACKEND SESSION WEBSOCKET TRANSPORT");
  Serial.println("==========================================");

  // Hardware identity remains stable; the boot identity changes at reboot.
  // Monitoring session identity is supplied later by backend START.
  ...
  snprintf(
      ...
      sizeof(deviceId),
      "ESP32-%04lX%08lX",
      (unsigned long)((chip >> 32) & 0xFFFFULL),
...
void loop() {
  ...
  if (now >= nextBatch) {
    ...
    bool shouldTransmit;
    uint32_t sessionGenerationSnapshot;
    char sessionIdSnapshot[sizeof(sessionId)];
    ...
    sessionGenerationSnapshot = sessionGeneration;

    if (shouldTransmit) {
      memcpy(sessionIdSnapshot, sessionId, sizeof(sessionIdSnapshot));
      sessionIdSnapshot[sizeof(sessionIdSnapshot) - 1] = '\0';
    } else {
      sessionIdSnapshot[0] = '\0';
    }

    portEXIT_CRITICAL(&networkMux);

    // Acquisition buffers are always drained above. Serialization and
    // transmission are intentionally skipped while no backend session is
    // active, so idle operation is not reported as a network failure.
    if (shouldTransmit) {
      size_t jsonLength = 0;
  ...
  if (now >= nextDiagnostic) {
    ...
    wsErr = wsErrorEvents;
    helloCount = helloEvents;
    helloFailures = helloFailuresTotal;
    ...
    Serial.printf(
        "NET WIFI:%s WS:%s READY:%u STREAM:%u | "
        ...
        "HELLO:%lu HELLOFAIL:%lu | "
        "WS_EVT C:%lu D:%lu E:%lu\n",

        wifiUp ? "UP" : "DOWN",
        wsUp ? "UP" : "DOWN",
        backendIsReady ? 1U : 0U,
        streamIsActive ? 1U : 0U,
        ...
        (unsigned long)helloCount,
        (unsigned long)helloFailures,
